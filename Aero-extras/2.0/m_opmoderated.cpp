...
{
 public:
	OpModeratedMode(Module* Creator) : SimpleChannelModeHandler(Creator, "opmoderated", 'U') { }
};
...
class ModuleOpModerated : public Module
{
 ...
 public:
	...
	void Prioritize()
	{
		// since we steal the message, we should be first, not last (let noone else eat it first)
		ServerInstance->Modules->SetPriority(this, I_OnUserPreMessage, PRIORITY_FIRST);
		ServerInstance->Modules->SetPriority(this, I_OnUserPreNotice, PRIORITY_FIRST);
	}
};
