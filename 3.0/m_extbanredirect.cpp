...
class BanWatcher : public ModeWatcher
{
 ...
 public:
	...
	bool BeforeMode(User* source, User*, Channel* channel, std::string& param, bool adding) CXX11_OVERRIDE
	{
		...
		{
			source->WriteNumeric(ERR_REDIRECT, InspIRCd::Format("Extban redirect \"%s\" is invalid. Format: %c:<chan>:<mask>", param.c_str(), extbanchar));
			return false;
		}
		...
		{
			source->WriteNumeric(ERR_NOSUCHCHANNEL, channel->name, InspIRCd::Format("Invalid channel name in redirection (%s)", targetname.c_str()));
			return false;
		}
		...
		if (!targetchan)
		{
			source->WriteNumeric(ERR_NOSUCHCHANNEL, channel->name, InspIRCd::Format("Target channel %s must exist to be set as a redirect.", targetname.c_str()));
			return false;
		}
		...
		{
			source->WriteNumeric(ERR_CHANOPRIVSNEEDED, channel->name, InspIRCd::Format("You must be opped on %s to set it as a redirect.", targetname.c_str()));
			return false;
		}
...
class ModuleExtBanRedirect : public Module
{
 ...
 public:
	...
	ModResult OnCheckBan(User* user, Channel* chan, const std::string& mask) CXX11_OVERRIDE
	{
		...
		{
			if (target->IsModeSet(limitredirect) && target->GetUserCounter() >= ConvToNum<size_t>(target->GetModeParameter(limitmode)))
			{
